-- Migration 107 : ajout de credit_attente_411 dans v_lignes_bancaires_avec_statut
--
-- Problème : restant = credit - SUM(tous lettrages) inclut le lettrage technique
-- 411_ATTENTE (+X), donc restant = 0 dès l'affectation, rendant le montant
-- "à dispatcher" illisible dans la vue Compte.
--
-- Solution : ajouter credit_attente_411 = somme algébrique des lettrages 411_ATTENTE
-- (initial +X − corrections partielles). Ce champ donne le crédit réel restant
-- à dispatcher indépendamment du restant comptable de la ligne.
...
SELECT
  ...
  COALESCE(SUM(CASE WHEN l.numero_facture = '411_ATTENTE' THEN l.montant ELSE 0 END), 0)::numeric(12,2) AS credit_attente_411,
  lb.organisation_id
