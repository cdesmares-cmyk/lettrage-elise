-- Migration 159 : niveau_relance + est_gele dans alertes_score
--
-- niveau_relance (0/1/2/3) : nombre de cycles de relance "sans suite" sur les
--   factures actuellement ouvertes de ce client.
--   0 = aucune relance envoyée sur ces factures
--   1 = une relance envoyée > 30 j sans paiement  → suggestion scénario N1
--   2 = deux relances sans suite                  → suggestion scénario N2
--   3 = trois relances ou plus sans suite         → suggestion mise en demeure N3
--
-- est_gele : client en procédure collective (sauvegarde/redressement/liquidation)
--   ou avec alerte BODACC active. Hors du cycle de relance standard.
--
-- Règle d'overlap : une relance est "sur ces factures" si son tableau factures_ids
--   a au moins un élément en commun avec les factures actuellement ouvertes (&&).
-- Règle "sans suite" : relance envoyée + ancienneté > 30 j + au moins une des
--   factures concernées est encore impayée aujourd'hui (c'est-à-dire toujours dans
--   encours_actifs, ce que le JOIN garantit implicitement).
...
BEGIN
  ...
  -- Niveau de relance : compte les relances envoyées depuis > 30 j dont au moins
  -- une facture est encore impayée aujourd'hui (overlap &&).
  ...
  niveau_relance_client AS (
    ...
    JOIN relances r
      ...
      AND r.envoyee_le       < (v_today - INTERVAL '30 days')::timestamptz
