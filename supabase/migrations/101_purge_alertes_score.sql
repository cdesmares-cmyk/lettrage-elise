-- Migration 101 : purge automatique des anciennes lignes alertes_score
--
-- Problème : calculer_scores_org ne supprimait que les lignes du jour courant
-- avant recalcul. Les lignes des jours précédents s'accumulaient indéfiniment.
-- Observé le 2026-07-21 : 22 610 lignes en 43 jours (~526/jour).
-- La vue v_comptes_clients jointure sur date_calcul = CURRENT_DATE uniquement —
-- les lignes passées ne sont jamais lues.
--
-- Corrections :
--   1. Purge immédiate de toutes les lignes antérieures à aujourd'hui
--   2. Ajout d'un DELETE historique dans calculer_scores_org (une ligne)

-- Purge immédiate
DELETE FROM alertes_score WHERE date_calcul < current_date;

-- Mise à jour de la fonction : ajout de la purge de l'historique avant le recalcul
-- (version de base : migration 089 — seuil âge 5 ans)
...
BEGIN
  ...
  -- Purge des lignes antérieures à aujourd'hui pour cette org
  DELETE FROM alertes_score
  WHERE organisation_id = p_organisation_id AND date_calcul < v_today;

  -- Recalcul du jour : supprime les lignes d'aujourd'hui avant réinsertion
  DELETE FROM alertes_score
  WHERE organisation_id = p_organisation_id AND date_calcul = v_today;
