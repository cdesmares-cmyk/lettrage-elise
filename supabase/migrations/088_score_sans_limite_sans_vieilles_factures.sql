-- Migration 088 : score client — suppression LIMIT 20 + filtre factures > 3 ans
--
-- Problèmes corrigés :
--   1. LIMIT 20 par org rendait le score invisible pour les orgs > 20 clients éligibles
--   2. Les factures anciennes (> 3 ans) gonflaient artificiellement le score
--      de clients aux vieilles créances irrécouvrables, masquant les vrais risques actifs
...
DECLARE
  ...
  v_seuil_age  DATE := current_date - INTERVAL '3 years';
  v_nb         INT;
BEGIN
  ...
  -- Toutes les factures du client (soldées + impayées, < 3 ans) pour le ratio global
  ...
  -- Factures impayées < 3 ans uniquement
  encours_actifs AS (
    SELECT
      f.code_client,
  ...
  -- LIMIT supprimé — tous les clients éligibles sont scorés
