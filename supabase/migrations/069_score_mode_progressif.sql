-- Migration 069 : score progressif — mode A (factures ouvertes) + mode B (lettrages)
--
-- Problème : pour une nouvelle organisation sans historique lettrage,
-- pts_retard / pts_rupture / pts_tendance valent tous 0 → score sous-évalué.
--
-- Solution : enrichir encours_actifs avec deux signaux instantanés basés sur
-- les factures ouvertes uniquement (mode A), utilisés en fallback quand
-- l'historique lettrage est absent ou insuffisant.
--
-- Nouveaux signaux mode A :
--   delai_moyen_ouvert  — délai moyen des factures échues non soldées (proxy retard_3m)
--   ratio_impayees      — nb factures impayées / nb total factures client
--
-- Pondération progressive :
--   pts_retard   → COALESCE(r3.moy, delai_moyen_ouvert, 0)  : mode B si dispo, sinon mode A
--   pts_ratio    → signal pur mode A, toujours actif (remplace l'ancien pts_echu)
--                  nb_echu / nb_total_all (toutes factures, pas seulement impayées)
...
BEGIN
  ...
  -- Toutes les factures du client (soldées + impayées) pour le ratio global
  ...
  encours_actifs AS (
    SELECT
      ...
      -- Mode A — délai moyen des factures échues ouvertes (fallback si pas de lettrages)
  ...
  scores AS (
    SELECT
      ...
      -- pts_retard : mode B (lettrages) si dispo, sinon mode A (factures ouvertes)
      LEAST(30, GREATEST(0,
        CASE
      ...
      -- pts_rupture : mode B uniquement (nécessite un historique de référence)
      CASE
        ...
        ELSE                                               0
      END AS pts_rupture,
      -- pts_tendance : mode B uniquement (régression nécessite ≥ 3 lettrages)
      LEAST(20, GREATEST(0, COALESCE(t.pente, 0) * 2))  AS pts_tendance,
      -- pts_echu : mode A — ratio factures échues / total factures client (toujours actif)
