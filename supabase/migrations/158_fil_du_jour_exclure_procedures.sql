-- Migration 158 : Exclure procédures collectives et BODACC du Fil du jour
--
-- Un client en liquidation/redressement/sauvegarde ou avec BODACC actif
-- n'est pas actionnable par une relance standard → score_fil_du_jour = 0.
-- Le score_risque reste inchangé (utile pour le suivi et les alertes).
...
BEGIN
  ...
  SELECT
    ...
    -- Procédure collective ou BODACC = non-actionnable par relance → exclu du fil du jour
    CASE
