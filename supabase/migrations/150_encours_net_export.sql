-- Migration 150 : ajout de encours_net dans v_comptes_clients
-- encours_net = somme algébrique de tous les soldes non nuls (factures + avoirs + 411)
-- encours_total reste inchangé (compatibilité UI)
...
SELECT
  ...
  COALESCE(
    SUM(CASE WHEN ABS(f.reste_du) > 0.005 THEN f.reste_du ELSE 0 END),
    0
  )::numeric(12,2)                                                              AS encours_net,
...
GROUP BY
  ...
  c.relance_auto_alerte, c.note_client, c.a_suivre;
