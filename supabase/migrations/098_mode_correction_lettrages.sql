-- Migration 098 : Ajout mode 'correction' + correction_id pour les lignes de correction de lettrage.
-- Les corrections sont identifiables par mode='correction' et regroupées par correction_id (UUID partagé).
-- creer_export_comptable est étendu pour verrouiller les corrections au même titre que les compensations.

-- 1. Mettre à jour la contrainte CHECK sur mode
...
ALTER TABLE lettrages ADD CONSTRAINT lettrages_mode_check
  CHECK (mode IN ('auto', 'semi', 'manuel', 'import', 'remboursement', 'compensation', 'correction'));

-- 2. Ajouter la colonne correction_id (UUID, nullable)
ALTER TABLE lettrages ADD COLUMN IF NOT EXISTS correction_id uuid;
CREATE INDEX IF NOT EXISTS idx_lettrages_correction_id ON lettrages(correction_id) WHERE correction_id IS NOT NULL;

-- 3. Étendre creer_export_comptable pour verrouiller les corrections de la période
...
BEGIN
  ...
  -- Verrouiller les corrections de la période (non encore exportées)
  WITH corr_ids AS (
  ...
  verrouilles_corr AS (
    ...
    SET    export_id = v_export_id
    WHERE  organisation_id  = v_org_id
      AND  correction_id    IN (SELECT correction_id FROM corr_ids)
      AND  annule           = false
      AND  export_id        IS NULL
    RETURNING id
  )
  SELECT COUNT(*)::integer INTO v_nb_corr FROM verrouilles_corr;

  -- Annuler si rien à verrouiller (tout déjà exporté ou aucune ligne éligible)
  IF v_nb = 0 AND v_nb_comp = 0 AND v_nb_corr = 0 THEN
    DELETE FROM exports_comptables WHERE id = v_export_id;
    RAISE EXCEPTION 'Aucune ligne éligible : toutes les lignes 100%% lettrées, compensations et corrections de cette période sont déjà verrouillées';
  END IF;

  UPDATE exports_comptables
  SET    nb_lettrages = v_nb + v_nb_comp + v_nb_corr, montant_total = v_montant
