-- Migration 036 : table alertes_score + RPC calculer_scores_org

CREATE TABLE IF NOT EXISTS alertes_score (
...
CREATE INDEX IF NOT EXISTS idx_alertes_score_org_date
  ON alertes_score (organisation_id, date_calcul DESC);

-- RPC : calcule et stocke le top 20 des clients à risque pour une organisation donnée.
-- Scorecard multi-critères :
--   30% retard moyen 3 mois (lettrages réels vs date_echeance)
--   20% rupture comportement (retard_3m / retard_12m)
--   20% tendance (pente régression linéaire sur 6 derniers lettrages)
--   20% % factures échues au-delà du seuil
--   10% alerte BODACC active
...
BEGIN
  ...
  -- Recalcul quotidien : on écrase les lignes du jour
