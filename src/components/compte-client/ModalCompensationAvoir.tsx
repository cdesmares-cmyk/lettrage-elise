...
// Flow : onglet Nouvelle (sélection avoir → factures → validation) | onglet Historique (annulation)
...
function TabHistorique({ codeDso, orgId, compensation, onRefreshFactures }: {
  ...
  async function handleAnnuler(id: string) {
    await compensation.annulerCompensation(id, () => { onRefreshFactures(); charger() })
    setConfirmId(null)
  }

  if (charge) return <div className="flex items-center justify-center py-12 text-xs text-gray-400">Chargement…</div>
  if (groupes.length === 0) return <div className="flex items-center justify-center py-12 text-xs text-gray-400 italic">Aucune compensation enregistrée pour ce client.</div>

  return (
    <div className="p-4 space-y-3">
      {groupes.map(g => (
        <div key={g.id} className={`rounded-xl border p-3 ${g.annule ? 'bg-gray-50 border-gray-200 opacity-60' : 'bg-white border-gray-200'}`}>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-1.5">
              {g.annule && <span className="text-[9px] font-bold uppercase tracking-wider text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded-full">Annulée</span>}
              {!g.annule && g.exportee && <span className="text-[9px] font-bold uppercase tracking-wider text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-full">Exportée</span>}
              {!g.annule && !g.exportee && confirmId !== g.id && (
                <button onClick={() => setConfirmId(g.id)} className="text-[9px] font-semibold text-red-500 hover:text-red-700 px-1.5 py-0.5 rounded hover:bg-red-50 transition-colors">
                  Annuler
                </button>
              )}
              {confirmId === g.id && (
                <div className="flex items-center gap-1.5">
                  <span className="text-[9px] text-gray-500">Confirmer ?</span>
                  <button onClick={() => handleAnnuler(g.id)} className="text-[9px] font-bold text-red-600 hover:underline">Oui</button>
                  <button onClick={() => setConfirmId(null)} className="text-[9px] text-gray-400 hover:underline">Non</button>
                </div>
              )}
            </div>
          </div>
          <div className="space-y-1">
            {g.avoir && (
              <div className="flex items-center justify-between px-2 py-1 rounded-lg bg-ockham-teal/5">
                ...
                <span className="text-xs font-bold text-ockham-teal tabular-nums">+{fmt(Math.abs(g.avoir.montant))}</span>
              </div>
            )}
            {g.factures.map(f => (
    ...
    <>
      <div className="flex-1 overflow-y-auto">
        <div className="px-4 pt-4 pb-2">
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Avoirs disponibles</p>
                ...
                {listeAvoirs.map(f => {
                  ...
                  return (
                    <button key={f.numero_piece} onClick={() => compensation.selectionnerAvoir(f)}
                      className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg border text-left transition-all cursor-pointer ${isSource ? 'border-ockham-teal bg-ockham-teal/5 ring-1 ring-ockham-teal/30' : 'bg-white border-gray-200 hover:border-ockham-teal hover:bg-ockham-teal/5'}`}>
                      <div>
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                        <span className="text-sm font-bold tabular-nums text-ockham-teal">{fmt(Math.abs(f.reste_du))}</span>
                        {isSource && <span className="text-[9px] font-bold uppercase tracking-wider text-ockham-teal bg-ockham-teal/10 px-1.5 py-0.5 rounded">Source</span>}
                      </div>
                    </button>
                  )
                })}
        ...
        {avoirSource && (
          <div className="px-4 pt-4 pb-2 border-t border-gray-100 mt-2">
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Factures à compenser</p>
