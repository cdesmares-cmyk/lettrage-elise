...
export function BarreKpis({ kpis, chargement }: Props) {
  ...
  return (
    <div className={`grid grid-cols-4 gap-3 mb-5 ${cls}`}>
      <Kpi label="Clients actifs" valeur={String(kpis.nbClientsActifs)} sous="avec encours > 0" />
      <Kpi label="Encours total TTC" valeur={fmt(kpis.encoursTotalTtc)} couleur="warning" />
...
function Kpi({ label, valeur, sous, couleur }: { label: string; valeur: string; sous?: string; couleur?: 'warning' | 'danger' }) {
  const valCls = couleur === 'danger' ? 'text-red-600' : couleur === 'warning' ? 'text-amber-600' : 'text-gray-900'
  return (
