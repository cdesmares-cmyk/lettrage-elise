// Modale compensation crédit — transfert du surpaiement d'une facture vers une/plusieurs factures du même client
...
const filtreSurpayees = (fs: FactureDetail[]) =>
  fs.filter(f => !f.est_avoir && f.reste_du < -TOLERANCE_CENT && !f.numero_piece.startsWith('411_'))
...
// ── Overlay confirmation annulation (identique à ModalCompensationAvoir) ──────
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm p-6 space-y-4">
        ...
        <p className="text-xs text-gray-500">Les soldes des factures concernées seront restaurés.</p>
        <textarea
          value={motif}
...
function TabNouvelle({ factures, onValide }: {
  ...
  const surpayees = filtreSurpayees(factures)
  ...
  function motifInvalide(): string | null {
    if (!source) return 'Aucune facture source sélectionnée'
  ...
  async function valider() {
    ...
    try {
      ...
      const rows = [
        {
          ...
          mode: 'compensation',
          commentaire: `Compensation crédit → ${refDests}`,
          operateur,
      ...
      toast.success(`Compensation crédit enregistrée, ${nb} facture${nb > 1 ? 's' : ''} concernée${nb > 1 ? 's' : ''}`)
      setSource(null)
      setSelection([])
      onValide()
    } catch (err) {
      ...
      <div className="flex-1 overflow-y-auto">
        {!source ? (
          <div className="px-4 pt-4 pb-4">
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Étape 1 — Sélectionnez une facture surpayée</p>
            {surpayees.length === 0
              ? <p className="text-xs text-gray-400 italic px-1">Aucune facture surpayée sur ce compte.</p>
...
export function ModalCompensationCredit({ codeDso, nomClient, factures, onFermer, onRefreshFactures }: Props) {
  ...
  const nbSurpayees = filtreSurpayees(factures).length
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 flex-shrink-0"
          ...
          <div>
            <p className="text-sm font-bold text-white">Compensation crédit — <span className="text-ockham-teal">{nomClient}</span></p>
            <p className="text-[10px] text-white/40 mt-0.5 font-mono">{codeDso} · {nbSurpayees} facture{nbSurpayees !== 1 ? 's' : ''} surpayée{nbSurpayees !== 1 ? 's' : ''}</p>
          </div>
