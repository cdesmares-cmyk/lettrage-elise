  ...
  return (
    <>
      ...
      <div className="fixed top-0 right-0 bottom-0 w-[380px] bg-white shadow-2xl z-50 flex flex-col">
        ...
        {/* Corps — conditionnel selon onglet */}
        {onglet === 'infos' ? (
          <>
            <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
                ...
                <div className="space-y-1.5">
                  {STATUTS.map(s => (
                    <button
                      ...
                      className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg border text-xs font-semibold transition-all cursor-pointer ${
                  ...
                  {statut && (
                    <button onClick={() => setStatut(null)} className="w-full text-xs text-gray-400 hover:text-gray-600 py-1 transition-colors cursor-pointer">
                      ✕ Effacer le statut
                    </button>
                  )}
                </div>
              ...
              <div>
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg border text-sm font-medium transition-all cursor-pointer ${
                    nePasRelancer
            ...
            {/* Footer — uniquement onglet Infos */}
            <div className="flex gap-2 px-5 py-4 border-t border-gray-100 flex-shrink-0">
              <button
                onClick={onFermer}
                disabled={enregistrement}
                className="flex-1 text-sm font-medium text-gray-500 border border-gray-200 py-2.5 rounded-lg hover:border-gray-300 transition-colors disabled:opacity-40 cursor-pointer"
              >
                Annuler
              </button>
              <button
                ...
                disabled={enregistrement}
                className="flex-[2] flex items-center justify-center gap-2 bg-ockham-teal hover:bg-ockham-teal-dark disabled:opacity-40 text-white text-sm font-semibold py-2.5 rounded-lg transition-colors cursor-pointer"
              >
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            <CommentairesFil contexte="facture" contexteId={facture.numero_piece} />
          </div>
        )}
      </div>
    </>
