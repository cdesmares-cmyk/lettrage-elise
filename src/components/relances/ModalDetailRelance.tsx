      ...
      >
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-start justify-between gap-4 flex-shrink-0">
          <div className="min-w-0">
            <h2 className="text-base font-bold text-ockham-navy truncate">{nomClient}</h2>
            <div className="flex items-center gap-2 mt-1">
              <span className="font-mono text-[11px] text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded">{relance.code_client}</span>
              {relance.envoyee_le && (
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {jours !== null && (
              <span className={`text-[11px] font-bold px-2 py-1 rounded-lg border ${enRetard ? 'bg-amber-50 text-amber-600 border-amber-200' : 'bg-gray-50 text-gray-500 border-gray-200'}`}>
                {jours === 0 ? 'Auj.' : `J+${jours}`}
              </span>
            )}
            <button
              onClick={onFermer}
              className="w-7 h-7 rounded-lg border border-gray-200 flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-colors text-lg leading-none"
            >
              ×
            </button>
          </div>
              ...
              return (
                <div key={f.numero_piece} className="border border-gray-100 rounded-xl p-3 bg-gray-50/50 flex flex-col gap-2.5">
                  ...
                  <button
                    ...
                    className="self-end text-[10px] font-semibold px-2.5 py-1 rounded-lg bg-ockham-teal text-white hover:bg-ockham-teal-dark disabled:opacity-50 transition-colors"
                  >
          ...
          <div className="flex-1 px-5 py-4 overflow-y-auto flex flex-col gap-5">
            ...
            <div>
              ...
              <div className="flex items-start gap-0 mb-3">
                {PIPELINE.map((step, i) => {
                  ...
                  const canClick = peutModifier && !statutSaving && transitions.includes(step)
                  return (
                    <div key={step} className="flex items-center flex-1">
                      ...
                      >
                        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center text-[10px] font-bold transition-all ${
                          ...
                          canClick? 'bg-white border-gray-300 text-gray-400 group-hover:border-ockham-teal group-hover:text-ockham-teal cursor-pointer' :
                                    'bg-white border-gray-200 text-gray-300 cursor-default'
                        }`}>
                          {done ? '✓' : i + 1}
                        </div>
                        <span className={`text-[9px] font-semibold text-center leading-tight ${
                          done ? 'text-ockham-navy' : active ? 'text-ockham-teal' : canClick ? 'text-gray-400 group-hover:text-ockham-teal' : 'text-gray-200'
                        }`}>
                          {LABELS_PIPELINE[i]}
                        </span>
                      </button>
              ...
              {/* Sans réponse */}
              {peutModifier && transitions.includes('sans_reponse') && (
                <button
                  disabled={statutSaving}
                  onClick={() => changerStatut('sans_reponse')}
                  className="w-full flex items-center gap-2 px-3 py-2 rounded-lg border border-amber-200 bg-amber-50 hover:bg-amber-100 text-amber-700 transition-colors disabled:opacity-50"
                >
                    ...
                    <div className="ml-auto flex gap-1.5">
                      {transitions.map(t => (
                        <button key={t} disabled={statutSaving} onClick={() => changerStatut(t)}
                          className="text-[10px] font-semibold px-2 py-0.5 rounded border border-gray-200 bg-white hover:border-ockham-teal hover:text-ockham-teal transition-colors disabled:opacity-50">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  disabled={noteSaving}
                  onClick={async () => { setNoteSaving(true); await onSauvegarderNote(relance.id, noteTexte); setNoteSaving(false) }}
                  className="text-[10px] font-semibold px-3 py-1.5 rounded-lg bg-ockham-teal text-white hover:bg-ockham-teal-dark disabled:opacity-50 transition-colors"
                >
        ...
        <div className="px-6 py-3 border-t border-gray-100 bg-gray-50/60 flex items-center justify-between flex-shrink-0">
          <button
            ...
            className="flex items-center gap-1.5 text-[11px] font-semibold text-gray-400 border border-gray-200 bg-white hover:border-red-300 hover:text-red-500 hover:bg-red-50 px-3 py-1.5 rounded-lg transition-colors"
          >
          ...
          <button
            onClick={onFermer}
            className="text-[11px] font-semibold text-ockham-navy border border-ockham-navy/20 bg-white hover:bg-ockham-navy hover:text-white px-4 py-1.5 rounded-lg transition-colors"
          >
            Fermer
          </button>
        </div>
