          ...
          {recentes.map(r => {
            ...
            return (
              <tr key={r.id} className="hover:bg-gray-50/40 transition-colors">
                ...
                <td className="px-4 py-3 text-gray-700 max-w-[240px] truncate">{r.objet}</td>
                <td className="px-4 py-3 text-gray-400 text-xs tabular-nums">
                  {r.envoyee_le ? fmtDate(r.envoyee_le) : '—'}
                </td>
                <td className="px-4 py-3">{badgeStatut(r.statut)}</td>
                        ...
                        {actions.map(a => {
                          ...
                          return (
                            <button
                              ...
                              className="text-[10px] font-semibold px-2 py-1 rounded border border-gray-200 text-gray-500 hover:border-blue-300 hover:text-blue-600 hover:bg-blue-50 transition-colors"
                            >
                              → {s.label}
                            </button>
                          )
