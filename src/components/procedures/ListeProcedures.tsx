...
export function ListeProcedures({ lignes, chargement, onOuvrirDetail }: Props) {
  ...
  // Reset page quand tri, filtre, recherche ou données changent
  useEffect(() => { setPage(0) }, [colSort, dirSort, typesExclus, recherche, lignes])
  ...
  const lignesRecherche = r
    ? lignesFiltres.filter(l => l.codeClient.toLowerCase().includes(r) || l.nom.toLowerCase().includes(r))
    : lignesFiltres
  const lignesTri     = trier(lignesRecherche, colSort, dirSort)
  ...
  return (
    <div>
      ...
      {/* Barre d'outils — recherche + filtre type */}
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-gray-50">

        {/* Recherche code client / nom */}
        <div className="relative">
          <svg className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <circle cx="10.5" cy="10.5" r="6.5"/><path d="M15.5 15.5L19 19"/>
          </svg>
          <input
            type="text"
            value={recherche}
            onChange={e => setRecherche(e.target.value)}
            placeholder="Code client, nom…"
            className="pl-8 pr-3 py-1.5 text-xs border border-gray-200 rounded-lg bg-white text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-ockham-teal/30 focus:border-ockham-teal w-52 transition-colors"
          />
        </div>
