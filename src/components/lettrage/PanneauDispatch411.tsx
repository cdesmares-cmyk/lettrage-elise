...
export function PanneauDispatch411(props: Props) {
  ...
  if (!factureActive) {
    return (
      <div className="bg-white border border-indigo-100 rounded-xl shadow-sm flex flex-col items-center justify-center min-h-[320px] text-center px-6 py-10 sticky top-20">
        <div className="mb-3 opacity-20 text-indigo-400"><IcCursor size={40} /></div>
        <p className="font-semibold text-gray-700 text-sm mb-1">Sélectionnez un compte 411</p>
        <p className="text-gray-400 text-xs">Cliquez sur une entrée 411 pour la dispatcher</p>
      </div>
    )
  }
  ...
  return (
    <div className="bg-white border border-indigo-100 rounded-xl shadow-sm overflow-hidden sticky top-20">
      {/* En-tête */}
      <div className="px-5 py-4 border-b border-indigo-100" style={{ background: 'linear-gradient(135deg, #EEF2FF 0%, #fff 60%)' }}>
        <p className="text-[10px] font-bold text-indigo-500 uppercase tracking-widest mb-2">Compte Client 411</p>
        <p className="text-2xl font-extrabold tabular-nums text-indigo-600 leading-none">{fmt(creditDisponible)}</p>
        <p className="text-sm font-semibold text-gray-800 truncate mt-2">
          {factureActive.nom_client ?? codeClient}
        </p>
        <p className="text-xs text-gray-400 mt-0.5 font-mono">{factureActive.numero_piece}</p>
      </div>

      {/* Warning multi-client — non bloquant */}
      {warningMultiClient && (
        <div className="mx-5 mt-4 flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2.5 text-xs text-amber-700">
          <IcWarning size={13} className="flex-shrink-0 mt-0.5 text-amber-500" />
      ...
      <div className="px-5 pt-2 pb-2">
        ...
        <button
          ...
          className="flex items-center gap-1.5 w-full border border-dashed border-gray-200 hover:border-indigo-400/40 hover:text-indigo-500 text-gray-400 text-xs font-medium py-2 rounded-lg transition-all"
        >
      ...
      <div className="mx-5 mb-4 bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 text-sm">
        <div className="flex justify-between items-center py-1">
          <span className="text-xs text-gray-400">Crédit 411</span>
        ...
        <div className="flex justify-between items-center py-1 border-t border-gray-200 mt-1 pt-2">
          ...
          <div className="flex items-center gap-2">
            <span className={`font-bold tabular-nums ${surPaiement ? 'text-red-600' : restant < 0.01 ? 'text-emerald-600' : 'text-indigo-500'}`}>
