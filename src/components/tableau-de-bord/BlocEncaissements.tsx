...
function TooltipCustom({ active, payload, label }: { active?: boolean; payload?: { name: string; value: number; fill: string }[]; label?: string }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-lg shadow-lg px-3 py-2.5 text-xs min-w-[160px]">
      ...
      {payload.map(p => (
        <div key={p.name} className="flex items-center justify-between gap-4">
          ...
          <span className="font-mono font-semibold text-gray-800 dark:text-gray-100 tabular-nums">{fmtEuro(p.value)}</span>
        </div>
      ))}
    </div>
  )
    ...
    <div className="bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-xl shadow-sm overflow-hidden">
      <div className="border-b border-gray-100 dark:border-slate-700 px-5 py-3 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          {afficherNm1 && evolution !== null && (
            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${evolution >= 0 ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-red-50 text-red-700 dark:bg-red-900/30 dark:text-red-400'}`}>
          ...
          <div className="flex gap-1">
            {PERIODES.map(p => (
              <button
                ...
                className={`text-[10px] font-semibold px-2.5 py-1 rounded transition-colors ${periodeEncaissement === p.val ? 'bg-slate-800 dark:bg-slate-600 text-white' : 'bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-slate-600'}`}
              >
                {p.label}
              </button>
            ))}
      ...
      <div className="p-5">
        {pointsEncaissement.every(p => p.courant === 0 && p.precedent === 0) ? (
          <div className="flex items-center justify-center h-48 text-xs text-gray-400 dark:text-gray-500">
